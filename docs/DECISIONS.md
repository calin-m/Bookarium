...
- **Decision**: Architect a comprehensive, non-breaking technical SEO and rate-shielding engine governed by four core principles:
  ...
  4. **Native Safe Structured Data (JSON-LD)**: Inject `schema.org/WebSite` (with Sitelinks SearchAction) on the root layout and `schema.org/Book` on reader pages using native React 19 text nodes, completely eliminating forbidden `dangerouslySetInnerHTML` primitives in compliance with Pass 0.5 security policies.
